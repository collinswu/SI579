...
function showRhymes(result) {
    if (result.length != 0) {
        ...
        for (let numSyllables in groupOutput) {
            ...
            for (let syllable of groupOutput[numSyllables]) {
                console.log(syllable);
                wordOutput.innerHTML += `<li>${syllable['word']} <button type="button" class="btn btn-outline-success" onclick="addToSavedWords('${syllable['word']}')">(Save)</button></li>`;
            }
            wordOutput.innerHTML += `</ul>`;
        }
    } else {
...
function showSynonyms(result) {
    if (result.length != 0) {
        ...
        for (let res of result) {
            console.log(res);
            wordOutput.innerHTML += `<li>${res['word']} <button onclick="addToSavedWords('${res['word']}')"  type="button" class="btn btn-outline-success">(Save)</button></li>`;
        }
        wordOutput.innerHTML += `</ul>`;
    } else {
        wordOutput.innerHTML = `<p>(no results)</p>`;
